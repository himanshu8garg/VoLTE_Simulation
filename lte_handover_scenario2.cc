...
void ThroughputMonitor (FlowMonitorHelper *fmhelper, Ptr<FlowMonitor> flowMon,Gnuplot2dDataset DataSet)
  {
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator stats = flowStats.begin (); stats != flowStats.end (); ++stats)
    {
      Ipv4FlowClassifier::FiveTuple fiveTuple = classing->FindFlow (stats->first);
       //if (fiveTuple.sourceAddress == Ipv4Address("7.0.0.2") ||  fiveTuple.sourceAddress == Ipv4Address("7.0.0.1") /*|| fiveTuple.sourceAddress == Ipv4Address("7.0.0.2") || fiveTuple.sourceAddress == Ipv4Address("7.0.0.3")*/)
      {
      ...
      Simulator::Schedule(Seconds(0.02),&ThroughputMonitor, fmhelper, flowMon,DataSet);
   //if(flowToXml)
      {
...
main (int argc, char *argv[])
{
  ...
  double speed = 40;       // m/s
  double simTime = 1.0;//(double)(numberOfEnbs + 1) * distance / speed; // 1500 m / 20 m/s = 75 secs
  ...
  //       uePosAllocator->Add(Vector(0,800,0));
  //   }
  ...
  //       Ptr<ConstantVelocityMobilityModel> p = ueNodes.Get(0)->GetObject<ConstantVelocityMobilityModel> ();
  ...
  for (uint32_t u = 0; u < numberOfUes; ++u)
    {
      ...
      //for (uint32_t b = 0; b < numBearersPerUe; ++b)
        //{
          ...
          std::cout<<ipv4RoutingHelper.GetStaticRouting (ue->GetObject<Ipv4> ());
